...
import org.leviatan.core.jdkutils.lists.ListsUtils;
...
public final class TextUtils {
     ...
     * @return concatenation
     */
    public static <T> String concatList(final List<T> listObject, final String separator, final Function<T, String> getString) {
     ...
     */
    public static String trunkInitialPart(final String delimiterToBegin, final String txt, final boolean includeDelimeter) {
    ...
    private static Map<String, String> buildMapLowTextVowelsReplacements() {
        ...
        mapReplacements.put("á", "a");
        mapReplacements.put("é", "e");
        mapReplacements.put("í", "i");
        mapReplacements.put("ó", "o");
        mapReplacements.put("ú", "u");

        mapReplacements.put("à", "a");
        mapReplacements.put("è", "e");
        mapReplacements.put("ì", "i");
        mapReplacements.put("ò", "o");
        mapReplacements.put("ù", "u");

        mapReplacements.put("â", "a");
        mapReplacements.put("ê", "e");
        mapReplacements.put("î", "i");
        mapReplacements.put("ô", "o");
        mapReplacements.put("û", "u");

        mapReplacements.put("ä", "a");
        mapReplacements.put("ë", "e");
        mapReplacements.put("ï", "i");
        mapReplacements.put("ö", "o");
        mapReplacements.put("ü", "u");
     ...
     * @param removeTags
     *            removeTags
     * @return the text without the text between tags
     */
    public static String removeTextBetweenTags(final String text, final String tagBegin, final String tagEnd, final boolean removeTags) {

        String textClean = text;

        while (true) {

            final String textIteration = removeOneTextBetweenTagsReturnNullForNoChange(textClean, tagBegin, tagEnd, removeTags);

            if (textIteration == null) {
                // No more changes
                return textClean;
            } else {
                textClean = textIteration;
            }
        }
    }

    private static String removeOneTextBetweenTagsReturnNullForNoChange(final String text, final String tagBegin, final String tagEnd,
            final boolean removeTags) {

        final int indexBegin = text.indexOf(tagBegin);

        if (indexBegin == -1) {
            return null;
        }

        final int indexEnd = text.indexOf(tagEnd, indexBegin + tagBegin.length());

        if (indexEnd == -1) {
            return null;
        }

        final int indexBeginEff = removeTags ? indexBegin : indexBegin + tagBegin.length();
        final int indexEndEff = removeTags ? indexEnd + tagEnd.length() : indexEnd;
        return text.substring(0, indexBeginEff) + text.substring(indexEndEff, text.length());
    }
}
